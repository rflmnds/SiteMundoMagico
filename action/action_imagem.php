<?php
	...
	if ($uploadOk != 0) {
		$sql = "SELECT * FROM foto(idItens, Endereco) WHERE idItens = " . $_GET['id'];
		...
		if(!isset($foto)){
			if (move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)) {
			    ...
			    $sql = "UPDATE foto SET Endereco ='" . basename($_FILES["img"]["name"]) .  "'";
			    $result = mysqli_query($conn, $sql);
			} 
			else {
			...
			if (move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)) {
			    $success = "The file " . basename( $_FILES["img"]["name"]) . " has been uploaded.";
			    $sql = "INSERT INTO foto(idItens, Endereco) VALUES (" . $_GET['id'] . ", '" . basename($_FILES["img"]["name"]) .  "')";
			    $result = mysqli_query($conn, $sql);
			} 
			else {
			    $success = "Sorry, there was an error uploading your file.";
